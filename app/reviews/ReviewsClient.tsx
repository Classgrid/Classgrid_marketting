                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {rev.isVerified && <BadgeCheck title="Verified Classgrid User" className="w-5 h-5 fill-[#1D9BF0] text-background shrink-0 -ml-0.5 cursor-help" />}
                          </div>
                          <p title="User's Organization or Institution" className="text-[11px] text-muted-foreground font-bold truncate tracking-tight cursor-help w-fit">{rev.institution}</p>
                        </div>
                      </div>
                      ...
                      {(rev.positives?.length || rev.moduleName) && (
                        <div className="flex flex-wrap gap-2 pt-4">
                          {rev.moduleName && rev.moduleName !== 'Overall' && (
                            <span title={`This user specifically reviewed our ${rev.moduleName} module.`} className="px-3 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold tracking-wider uppercase cursor-help">Module: {rev.moduleName}</span>
                          )}
                          {rev.positives?.filter(p => p && p.trim() !== '').map((p, i) => (
                            <span key={i} title="A key positive point mentioned by the user" className="px-3 py-1 rounded-lg bg-muted/30 dark:bg-white/[0.03] border border-border dark:border-white/5 text-muted-foreground text-[10px] font-bold tracking-wider uppercase cursor-help">{p}</span>
                          ))}
                        </div>
                      )}
