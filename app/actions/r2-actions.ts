...
/**
 * Server action to generate a pre-signed Cloudflare R2 URL for support images.
 */
export async function getPresignedUrlForSupportImage(fileName: string, mimeType: string) {
  const sanitizedName = fileName.replace(/[^a-zA-Z0-9._-]/g, "_");
  const timestamp = Date.now();
  const filePath = `support-images/${timestamp}_${sanitizedName}`;
