...
export async function POST(req: Request) {
  try {
    ...
    const gate = rateLimit({ key: `help-like:${ip}:${slug}`, max: LIKE_MAX_PER_WINDOW, windowMs: LIKE_WINDOW_MS });
    if (!gate.allowed) {
      return NextResponse.json({ error: "Rate limited" }, { status: 429, headers: { "Retry-After": String(gate.retryAfterSeconds) } });
    }

    const count = await incrementBlogCounter("blog_likes", helpSlug(slug));
    return NextResponse.json({ count }, { status: 200 });
  } catch (error) {
    console.error("Help Like API Error:", error);
...
export async function GET(req: Request) {
  try {
    ...
    const count = await getBlogCounterCount("blog_likes", helpSlug(slug));
    return NextResponse.json({ count }, { status: 200 });
  } catch (error) {
    console.error("Help Like GET Error:", error);
