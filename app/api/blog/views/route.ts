...
export async function POST(req: Request) {
  try {
    ...
    const gate = rateLimit({
      ...
      max: VIEW_MAX_PER_WINDOW,
      windowMs: VIEW_WINDOW_MS,
    });
    if (!gate.allowed) {
      return NextResponse.json(
        { error: "Too many view requests. Please retry shortly." },
        {
          status: 429,
    ...
    const count = await incrementBlogCounter("blog_views", slug);
...
export async function GET(req: Request) {
  try {
    ...
    const count = await getBlogCounterCount("blog_views", slug);
    return NextResponse.json({ count }, { status: 200 });
  } catch (error) {
