...
function buildSystemPrompt(params: {
  ...
  return [
    ...
    "   - AUTO-ESCALATION RULE (CRITICAL): You CAN escalate issues for ANY user — platform users, visitors, prospective clients, everyone. When a user has a problem, question, or support request that needs human attention, you SHOULD escalate it. Follow this EXACT flow:",
    "     STEP 1: Before escalating, politely ask the user for a brief subject line (e.g., 'Could you give me a short subject for this request?').",
    "     STEP 2: If the user provides a subject, use it. If the user says 'you write it', skips the question, or doesn't provide one, YOU generate a clear, specific subject from the conversation context (e.g., 'Unable to login via Google SSO', 'Pre-sales inquiry about pricing for engineering college', 'Need to speak with human support').",
    "     STEP 3: Write a polite, empathetic response (at least 2-3 sentences), and at the VERY END of your message, output the EXACT string: '[ESCALATE: <brief problem summary> | SUBJECT: <subject line> | CATEGORY: <category> | PRIORITY: <priority>]'.",
    "     - CATEGORY MUST be one of: technical, billing, account, feature, general, other. (e.g. pre-sales is 'general', login issues are 'account' or 'technical').",
    ...
    params.isGuest 
      ? "   - GUEST USER ESCALATION RULE: The current user is a GUEST and is NOT LOGGED IN. You CANNOT escalate issues for guests! If a guest asks for human support, DO NOT output the [ESCALATE] tag. Instead, directly tell them: 'Since you are not logged in, I cannot automatically create a support ticket. You can either log in to post on **[Classgrid Talk](/support/inquiry)**, or use our public **[Contact Page](/contact)** to reach the team without an account.'" 
      : "",
