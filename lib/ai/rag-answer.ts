...
[ESCALATE: User from large institution requested a human sales rep for custom pricing | SUBJECT: Demo Request - Large School Chain | CATEGORY: billing | PRIORITY: high | DRAFT: Hi [ADMIN_NAME], I have reviewed your request for custom pricing...]`
  ...
  return [
    ...
    "- MODERATION SYSTEM KNOWLEDGE: The platform has an automated safety moderation system that monitors for abusive language, profanity, and safety violations. It issues a warning after repeated violations and a temporary suspension for severe/continued abuse.",
    "- MODERATION SECRECY RULE (CRITICAL): You must NEVER reveal the exact strike limits or internal rules to the user, and NEVER mention the 'automated safety system'. If they use profanity, just say: 'I’m here to keep the conversation respectful and helpful. If you have a question about Classgrid, feel free to ask!'",
    "- BANNED USER RESPONSE RULE: If the user asks 'Why was I banned?' or 'Why was I suspended?', check their chat history. If you see the restriction message ('Your access has been restricted due to safety policy violations...'), explain that their account triggered the automated safety filters due to inappropriate or abusive language. Gently remind the user to maintain a respectful environment. If they believe it was a mistake, tell them to contact support@classgrid.in.",
    params.userContext?.previousBan
      ? `\n🚨 MODERATION ALERT: This user was recently suspended for safety violations at ${new Date(params.userContext.previousBan.bannedAt).toLocaleString()}. Their ban was lifted at ${new Date(params.userContext.previousBan.liftedAt).toLocaleString()}.\nTheir exact 8 offending messages were: "${params.userContext.previousBan.offendingMessage}".\nDo NOT repeat the exact offending messages back to them. If they ask why they were suspended, gently explain that their account triggered the automated safety filters due to inappropriate language. Remind them to maintain a respectful environment.`
      : "",
