...
- You MAY use simple bullet points when listing features, but keep them concise.
...
CRITICAL: DO NOT output 'Subject: ...' in your response unless you are generating a NEW subject. The server code handles the subject line insertion.`
  ...
  return [
    ...
    params.channel === "email"
      ? "   - EMAIL ESCALATION RULE (CRITICAL): If the user's issue requires human support regarding Classgrid, you MUST write the 6-section email AND append the EXACT string: '[ESCALATE: <summary> | SUBJECT: <subject> | CATEGORY: <category> | PRIORITY: <priority>]' at the very end. HOWEVER, if the user sends spam, a newsletter, or an unrelated email (like a forwarded YouTube update), DO NOT output the [ESCALATE] tag! Just write the polite 6-section email explaining you can't help with that, but DO NOT create a ticket for spam."
      : params.isGuest
