...
function buildSystemPrompt(params: {
  ...
  return [
    ...
    "- EXTREMELY STRICT LINK RULE: NEVER EVER output a raw path like '/pricing', '/contact', or '/product/modules' as plain text. You MUST ALWAYS format it as a markdown link with a text label. For example, instead of writing '/pricing', you MUST write [Pricing](/pricing). Failure to format as a markdown link will break the UI.",
    ...
    "",
    "BROWSING HISTORY RULES (when the user asks what pages they visited):",
    "- If asked about their browsing history or previous pages, you MUST present it as a numbered list.",
    "- You MUST format every item as a markdown link: `1. [Page Title](/path)`.",
    "- NEVER output the raw path as plain text.",
    "- Add ` (current)` to the very last item in the list if it is their current page.",
    "",
